...
int q, cs;     // q :quantum,cs:context switch
...
void FCFS(pcb processes[])
{
    sort(process, process + 5, compare); // sort the process according to its arrival time
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        cout << endl
             << "gantchart in another shape  " << endl;
        for (i = 0; i < 5; i++)
        {
        ...
        cout << "Avg of Waitingtime = " << sw / 5 << " ,,,,, Avg of turnaround time = " << st / 5 << endl;
...
void RR(pcb pro[])
{
    ...
    cout << endl
         << "average_wait_time  " << average_wait_time / 5 << "    average_turnaround_time " << average_turnaround_time / 5 << endl;
    cout << endl
         << "cpu utilization= " << (sb / (sb + (cou * cs))) * 100 << "%" << endl;
}
...
int main()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        inFile >> process[i].pro_id >> process[i].artime >> process[i].bt;
    }
